...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True,flip=True):
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True,shuffle=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
    transform_list = [transforms.RandomCrop((height, width))] + transform_list if crop else transform_list
...
def to_1_channel(A):
    for i in range(A.size()[0]):
        ...
        A = tmp.unsqueeze(0).unsqueeze(0)
        if i==0:
            final_im = A
        else:
            final_im = torch.cat([final_im,A],0)
    return final_im

def clipPatch(input_im,fineSize,encodeSize):
    clip_start_index = (fineSize - encodeSize) // 2
    clip_end_index = clip_start_index + encodeSize

    patch = input_im[:, :,clip_start_index:clip_end_index,
            clip_start_index:clip_end_index]
    out = pastePatch(patch,fineSize,encodeSize)
    return  out

def pastePatch(patch,fineSize,encodeSize):
    patch_num = int(fineSize/encodeSize)
    for i in range(patch_num):
        for j in range(patch_num):
            if j == 0:
                row_im = patch
            else:
                row_im = torch.cat((row_im, patch), 2)
        if i == 0:
            final_im = row_im
        else:
            final_im = torch.cat((final_im, row_im), 3)

    return final_im

def generateMaskPatch(mask,patch):
    mask =  mask*0.5 + 0.5
    final_im = patch*mask
