...
class MUNIT_Trainer(nn.Module):
    ...
    def forward(self, x_a, x_b):
        ...
        s_a_fake = self.enc_a(x_a)
        s_b_fake = self.enc_a(x_b)
        x_ba = self.gen_a(x_b, s_a)
        x_ab = self.gen_b(x_a, s_b)
        self.train()
        return x_ab, x_ba
    '''
...
class UNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        ...
        lr = hyperparameters['lr']
        # Initiate the networks
        self.gen_a = networks.VAEGen(hyperparameters['input_dim_a'], hyperparameters['gen'])  # auto-encoder for domain a
        self.gen_b = networks.VAEGen(hyperparameters['input_dim_b'], hyperparameters['gen'])  # auto-encoder for domain b
        self.dis_a = networks.MsImageDis(hyperparameters['input_dim_a'], hyperparameters['dis'])  # discriminator for domain a
        self.dis_b = networks.MsImageDis(hyperparameters['input_dim_b'], hyperparameters['dis'])  # discriminator for domain b
        self.instancenorm = nn.InstanceNorm2d(512, affine=False)
...
class myMUNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
    ...
    def gen_update(self, hyperparameters):
        ...
        x_b_recon = self.gen_a(x_ab, s_b_prime)
        # x_b_recon = self.gen_b(torch.cat((x_b_patch,x_b),1), s_b_prime) #mask
        self.x_a_recon = x_a_recon
        self.x_b_recon = x_b_recon
